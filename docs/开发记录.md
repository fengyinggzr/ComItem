...
### 7. ComQmlProgressBar 动画冲突问题 (2026-01-17)
**问题描述**:
- 在 `example/Main.qml` 中使用 `NumberAnimation on value` 驱动进度条动画时，`ComQmlProgressBar` 的宽度几乎不变化
- 不确定进度条在动画循环时会溢出右边界

**原因分析**:
1. **动画冲突**: `progressRect` 的 `Behavior on width` 与外部的 `NumberAnimation on value` 冲突
   - 当 `value` 持续快速变化时，`root.visualPosition` 也在持续变化
   - `Behavior on width` 会在每次宽度变化时启动 150ms 的动画
   - 新动画不断被触发，导致旧动画被中断，宽度变化极其缓慢
2. **溢出问题**: 不确定进度条动画的结束位置使用 `root.width` 而非 `root.availableWidth`，导致色块超出边界

**解决方案**:
1. 移除 `progressRect` 的 `Behavior on width`，让宽度直接跟随绑定实时更新
2. 将不确定进度条动画的结束位置从 `to: root.width` 改为 `to: root.availableWidth`
3. 在 `contentItem` 上添加 `clip: true` 确保溢出内容被裁剪

**修改文件**: `qml/ComQmlProgressBar.qml`
